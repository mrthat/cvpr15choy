\begin{figure}[t]
  ...
  % \end{tabular}\\
  % (b)
  \caption{Using a bank of CAD models, we generate NZ-WHO templates which can be used to either detecting objects directly or initializing pose of an object in the bounding box from other detectors. Once the initialization is given, we use NZ-WHO to propose and validate plausible pose on-the-fly and further tune the translation, 3D viewpoint and focal length continuously.}
  % \vspace{-1em}
  \label{fig:front}
\end{figure}
...
search in a continuos pose space, consisting of yaw, pitch and roll, resulting in truely continuous pose estimates, which
