...
\scream{Should we merge this with NZ-WHO?}
...
\scream{requires revision} Hariharan \etal. also interpreted the
...
\textcolor{red}{Should we mention how our method improves on this (ie by using CG)? Since we explain in depth later, it would be okay}
...
\scream{require revision: We propose Non-Zero Whitened Histograms of Orientations (NZ-WHO),
an improvement over WHO which is both more discriminative than WHO and can be
computed many orders of magnitude faster than WHO (70ms as opposed to to
several seconds for WHO). This speedup allows us to generate NZ-WHO templates
on the fly, allowing us to quickly evaluate arbitrary viewpoint hypotheses.}
...
do this? Nobody did this before} (see Fig.~\ref{fig:rendering}), but whitening the resulting HOG
...
Table \ref{tab:who_initializations} provides a detailed comparison of WHO and NZ-WHO templates, illustrating that our method is faster and more discriminative than Whitened Histograms of Orientations. \scream{Does the table show this? should we do more than AP and realtime analysis?}
...
% of the matrix (7500). As a result, conjugate gradient converges in 50
% ms, two orders of magnitude faster than the system can be solved using the
% Cholesky factorization (5 sec).
...
% implemented the convolution via Fast Fourier Transform on the GPU. 
% 
% This allows
