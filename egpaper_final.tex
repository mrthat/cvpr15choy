...
To whiten a signal $x$, we have to solve $\Sigma w = x - \mu$ which takes $O(n^3)$ time. In \cite{Hariharan12}, the authors make use of the fact that covariance matrices are Hermitian and positive semidefinite to solve the system via the Cholesky decomposition, which is twice as fast as the LU decomposition. However, by considering the properties of the covariance matrix of the HOG template, we show that the conjugate gradient method can further speed up the whitening stage.
...
Figure \ref{fig:spectral} shows the singular values of a template covariance matrix with a small regularization constant added to the diagonal. The singular values are clustered in small region and most of the energies are concentrated on the first \scream{x} singular values. Therefore, conjugate gradient will converge in $O(n^2\kappa)$ time, faster 
...
% R-CNN \cite{Girshick14} gives very irregular bounding boxes since the proposals bounding boxes come from various methods.
